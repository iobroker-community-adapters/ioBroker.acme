...
export const HTTP01_CHALLENGE_STATE = 'info.httpChallenges';
 ...
 */
export const HTTP01_CHALLENGE_PATTERN = `acme.*.${HTTP01_CHALLENGE_STATE}`;
 ...
 */
export const HTTP01_TOKEN_REGEX = /^[A-Za-z0-9_-]{16,128}$/;
...
export interface PublishedChallenge {
    ...
    /** ms epoch after which a reader must treat this entry as gone */
    expires: number;
}

/** Keyed by challenge token. */
export type PublishedChallenges = Record<string, PublishedChallenge>;
...
export interface ChallengePublisher {
    ...
    shutdown: () => void;
    /** Drop everything still published and wait for the write to land. */
    clear: () => Promise<void>;
}

interface PublisherConfig {
    log: ioBroker.Log;
    /** Persists the token map - the adapter supplies the states DB write. */
    write: (challenges: PublishedChallenges) => Promise<void>;
 ...
 */
class Http01ChallengePublisher implements ChallengePublisher {
    private readonly config: PublisherConfig;
    private readonly memdb: PublishedChallenges = {};
 ...
 */
export function create(config: PublisherConfig): ChallengePublisher {
...
export interface ProbeOptions {
    ...
    /** Configured bind address of the challenge server. */
    address: string;
    /** Configured port, i.e. where connections from the CA arrive. */
    port: number;
    log: ioBroker.Log;
    timeout?: number;
 ...
 */
export async function probeHttp01Endpoint(options: ProbeOptions): Promise<boolean> {
    const { publisher, address, port, log } = options;
    const doFetch = options.doFetch || fetch;

    const token = randomBytes(24).toString('base64url');
    const keyAuthorization = `${token}.${randomBytes(24).toString('base64url')}`;
