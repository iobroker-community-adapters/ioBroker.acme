...
function getDnsAuthorization(data) {
    const fromChallenge = data.challenge?.dnsAuthorization;
    const direct = data.dnsAuthorization;
    const value = fromChallenge || direct;
    if (!value) {
...
class AcmeDnsChallenge {
    ...
    async updateTxt(txt) {
        ...
        if (!response.ok) {
            ...
            throw new Error(`acme-dns update failed (${response.status}): ${bodyText.slice(0, 300)}`);
        }
    }
    async set(data) {
